...
static segment_t* ptr_segment(segment_t *segs, size_t numsegs, void *ptr)
{
    ...
    LOG("pointer out of range: " ADDR, (mach_vm_address_t)ptr);
    return NULL;
}

static mach_vm_address_t ptr_to_vmem(segment_t *segs, size_t numsegs, void *ptr)
{
...
static mach_vm_address_t vmem_find_bytes(segment_t *segs, size_t numsegs, void *search, size_t len, size_t granularity, char *name)
{
    for(size_t i = 0; i < numsegs; ++i)
    {
...
static mach_vm_address_t vmem_find_str(segment_t *segs, size_t numsegs, char *str)
{
...
mach_vm_address_t find_kernel_task(segment_t *text)
{
    ...
    mach_vm_address_t panic_info = vmem_find_str(text, 1, "aapl,panic-info");
    ...
    if(panic_info)
    {
        for(uint32_t *ptr = (uint32_t*)text->buf, *end = (uint32_t*)&((char*)ptr)[text->len]; ptr < end; ++ptr)
        {
            ...
            {
                mach_vm_address_t pc = ptr_to_vmem(text, 1, ptr);
                if(pc)
                {
                        ...
                        for(uint32_t *p = ptr - 1; p >= (uint32_t*)text->buf; --p)
                        {
                            if((*p & 0xffffffe0) == 0xd538d080) // mrs Xn, tpidr_el1
                            {
                                ...
                                if(num_ldrs == 1)
                                {
                                    ...
                                    if(pc)
                                    {
                                        mach_vm_address_t ret = pc + LDR_LIT(*last);
...
mach_vm_address_t find_ipc_space_kernel(segment_t *text)
{
    ...
    mach_vm_address_t str = vmem_find_str(text, 1, "\"failed to create resume port\"");
                                        ...
                                        if(IS_BL(*p))
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                )
                                                {
                                                    ...
                                                    if(pc)
                                                    {
                                                        mach_vm_address_t ret = (pc & 0xfffffffffffff000) + ADRP_LIT(p[-3]) + ADD_LIT(p[-2]) + LDR_IMM(p[-1]);
